...
export async function GET(request:Request){
  ...
  const mode:SimulationMode=url.searchParams.get("mode")==="study"?"study":"exam";
  if(!/^[a-z0-9-]+$/.test(slug))return noStore({error:"Simulado inválido."},400);

  try{
    const data=await supabaseAdminRpc<Record<string,unknown>>("prepare_simulation_v3",{
    ...
    if(message.includes("invalid_simulation_mode")) return noStore({error:"Modo de simulado inválido."},400);
...
export async function POST(request:Request){
  ...
  try{
    if(body.action==="answer"){
      ...
      // Estudo: tentativa salva usa ordem autoritativa do banco. Visitante só
      // fornece a ordem porque não existe tentativa persistida/ranking.
    ...
    if(body.action==="record"){
      // Tradicional: a resposta pode ser alterada enquanto a prova estiver
      // aberta, mas a API não retorna gabarito/correção antes da conclusão.
      if(!user?.id)return noStore({guest:true,recorded:true});
      const attemptId=positiveInteger(body.attempt_id);
      const questionId=positiveInteger(body.question_id);
      const selectedIndex=optionIndex(body.selected_index);
      if(!attemptId||!questionId||selectedIndex==null)return noStore({error:"Resposta inválida."},400);

      const result=await supabaseAdminRpc<Record<string,unknown>>("record_exam_answer",{
        p_user_id:user.id,
        p_attempt_id:attemptId,
        p_question_id:questionId,
        p_selected_index:selectedIndex,
    ...
    if(body.action==="finish"){
      ...
      const result=await supabaseAdminRpc<Record<string,unknown>>("finish_simulation_v3",{
        p_user_id:user.id,
        p_attempt_id:attemptId,
      });
      return noStore(result);
    ...
    if(message.includes("exam_answer_hidden")) return noStore({error:"O gabarito do simulado tradicional só é liberado após a conclusão."},409);
    ...
    if(message.includes("invalid_option")||message.includes("invalid_option_order")||message.includes("invalid_simulation_mode")) return noStore({error:"Resposta inválida."},400);
