...
export default async function CadastroPage({searchParams}:{searchParams:Promise<{error?:string}>}) {
  ...
  return <main className="admin-surface" style={{minHeight:"100vh",display:"grid",placeItems:"center",padding:24}}>
    <section style={{width:"min(480px,100%)",background:"#fff",padding:32,boxShadow:"0 24px 80px #07172d20"}}>
      ...
      {params.error&&<p style={{background:"#fff2f2",padding:"10px 12px",color:"#9c2d2d"}}>Não foi possível criar a conta. Verifique o e-mail e use uma senha com pelo menos 8 caracteres.</p>}
      <form method="post" action="/api/auth/signup" style={{display:"grid",gap:14}}>
        ...
        <label style={{display:"grid",gap:6,fontWeight:700}}>Senha<input name="password" type="password" minLength={8} required autoComplete="new-password" style={{padding:"12px 14px",border:"1px solid #d9e1e5"}}/></label>
