...
export default async function AdminCommunicationsPage(){
  const user=await requireAdminUser("/admin/comunicacoes");
  return <main className="admin-surface">
    ...
    <div className="admin-shell">
      <aside className="admin-sidebar"><nav aria-label="Navegação administrativa"><Link href="/admin"><ArrowLeft/>Visão geral</Link><Link className="active" href="/admin/comunicacoes"><Mail/>Comunicações</Link></nav><div className="admin-account"><span>KP</span><div><b>{user.fullName??"Kennedy Pereira"}</b><small>PROPRIETÁRIO</small></div></div></aside>
