...
export default async function AdminAnalyticsPage({searchParams}:{searchParams:SearchParams}){
 const user=await requireAdminPermission("analytics:view","/admin/analytics");
 ...
 return <main className="admin-surface">
  ...
  <div className="admin-shell">
   <aside className="admin-sidebar"><nav aria-label="Navegação administrativa"><Link href="/admin"><ArrowLeft/>Visão geral</Link><Link className="active" href="/admin/analytics"><BarChart3/>Analytics</Link><Link href="/admin/analytics/questoes"><BookOpenCheck/>Qualidade das questões</Link></nav><div className="admin-account"><span>{(user.fullName??user.email).slice(0,2).toUpperCase()}</span><div><b>{user.fullName??user.email}</b><small>{adminRoleLabel(user.role)}</small></div></div></aside>
   <section className="admin-content">
    <div className="admin-title"><div><span className="eyebrow"><span/> Inteligência operacional</span><h1>Analytics filtrável</h1><p>Tráfego, aprendizagem e qualidade do banco com o mesmo recorte de análise. A área financeira permanece zerada enquanto pagamentos estiverem adiados.</p></div><a className="button button-small" href={`/admin/analytics.csv?${exportQuery}`}><ArrowDownToLine size={16}/>Exportar CSV</a></div>
    ...
    <form method="GET" className="admin-panel" style={{marginBottom:24}}>
     ...
     {data.options.editions.length===0&&<p style={{margin:"14px 0 0",fontSize:12}}>O filtro por edição está habilitado e será preenchido conforme as questões recebam classificação de exame.</p>}
    </form>
