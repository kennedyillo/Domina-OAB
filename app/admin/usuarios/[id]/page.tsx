...
export default async function UserDetailPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="admin-surface"><header className="admin-header"><Brand compact/><div><span className="admin-environment"><i/> PAINEL ADMINISTRATIVO</span><a href="/api/auth/logout"><LogOut size={15}/>Sair</a></div></header><div className="admin-shell"><aside className="admin-sidebar"><nav><Link href="/admin/usuarios"><ArrowLeft/>Usuários</Link></nav><div className="admin-account"><span>KP</span><div><b>{user.fullName??"Kennedy Pereira"}</b><small>PROPRIETÁRIO</small></div></div></aside><section className="admin-content"><div className="admin-title"><div><span className="eyebrow"><span/> Histórico individual</span><h1>Desempenho do aluno</h1><p>Tentativas, evolução por disciplina e histórico de acesso.</p></div></div>
