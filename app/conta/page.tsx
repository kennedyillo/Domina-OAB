...
type Entitlement = { id:number; plan_id:string; status:string; starts_at:string; ends_at:string };
type Plan = { id:string; name:string; billing_type:string; price_cents:number };

function formatDate(iso:string){return new Date(iso).toLocaleDateString("pt-BR",{day:"2-digit",month:"long",year:"numeric"});}
function formatPrice(cents:number){return (cents/100).toLocaleString("pt-BR",{style:"currency",currency:"BRL"});}

export default async function ContaPage({searchParams}:{searchParams:Promise<{created?:string}>}) {
  ...
  try {
    const rows=await supabaseAdminSelect<Entitlement[]>(`entitlements?user_id=eq.${encodeURIComponent(user.id)}&status=eq.active&ends_at=gt.${encodeURIComponent(new Date().toISOString())}&select=id,plan_id,status,starts_at,ends_at&order=ends_at.desc&limit=1`);
    entitlement=rows[0]??null;
    if(entitlement){const planRows=await supabaseAdminSelect<Plan[]>(`plans?id=eq.${encodeURIComponent(entitlement.plan_id)}&select=id,name,billing_type,price_cents&limit=1`);plan=planRows[0]??null;}
  } catch {}
  ...
  return <main className="app-surface">
    ...
    <div className="app-container" style={{maxWidth:720,margin:"0 auto",padding:24}}>
      <span className="eyebrow"><span/> CONTA</span>
      <h1 style={{margin:"8px 0 24px"}}>Minha conta</h1>
      {params.created==="1"&&!entitlement&&<section style={{background:"#152844",border:"1px solid #31517c",borderRadius:14,padding:24,marginBottom:20}}><h2 style={{margin:"0 0 8px"}}>Conta criada. Agora escolha seu acesso.</h2><p style={{margin:"0 0 18px",color:"#aab8ca"}}>Criar uma conta não inscreve você automaticamente como fundador e não autoriza newsletter. Você pode tentar uma das 25 vagas gratuitas ou conhecer os planos pagos.</p><div style={{display:"flex",gap:12,flexWrap:"wrap"}}><a href="/ativar-fundador" className="button"><Gift size={16}/> Tentar acesso fundador</a><a href="/#planos" className="button button-light"><CreditCard size={16}/> Ver planos</a></div></section>}

      <section style={{background:"#0f1f38",border:"1px solid #1f3355",borderRadius:14,padding:24,marginBottom:20,display:"grid",gap:16}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:44,height:44,borderRadius:999,background:"#d4a13a",display:"grid",placeItems:"center",fontWeight:800,color:"#111"}}>{(fullName??user.email??"U").slice(0,1).toUpperCase()}</div><div><strong style={{display:"block",fontSize:18}}>{fullName??"Sem nome cadastrado"}</strong><span style={{color:"#8a9ab0",display:"flex",alignItems:"center",gap:6,fontSize:13}}><Mail size={14}/>{user.email}</span></div></div>
      </section>
