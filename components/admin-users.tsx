...
export function AdminUsers({canManage=false}:{canManage?:boolean}) {
  ...
  async function act(userId: string, action: string, days?: number) {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Erro na ação administrativa.");
    } finally {
      setBusy(null);
    }
  }
  ...
  const totals = useMemo(() => ({
    ...
    founders: users.filter((u) => u.founder_status === "active").length,
    blocked: users.filter((u) => u.account_status === "blocked").length,
  }), [users]);

  return <>
    <div className="admin-kpis">
      <article><span><UserRoundCheck/></span><div><small>USUÁRIOS</small><strong>{totals.total}</strong><p>na consulta atual</p></div></article>
      <article><span><CheckCircle2/></span><div><small>ATIVOS</small><strong>{totals.active}</strong><p>contas liberadas</p></div></article>
      <article><span><ShieldCheck/></span><div><small>FUNDADORES ATIVOS</small><strong>{totals.founders}</strong><p>benefício ativado</p></div></article>
      <article><span><Ban/></span><div><small>BLOQUEADOS</small><strong>{totals.blocked}</strong><p>acesso suspenso</p></div></article>
    </div>
      ...
      {loading ? <div className="admin-empty"><p>Carregando usuários...</p></div> : users.length===0 ? <div className="admin-empty"><p>Nenhum usuário encontrado.</p></div> : <div style={{display:"grid",gap:12}}>
        {users.map((user)=><article key={user.user_id} style={{border:"1px solid #e2ddd3",padding:18,background:"#fff"}}>
          <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1fr auto",gap:18,alignItems:"start"}}>
            <div><small style={{color:"#7b8189"}}>USUÁRIO</small><h3 style={{margin:"5px 0",color:"#0b1b33"}}>{user.full_name || "Sem nome cadastrado"}</h3><p style={{margin:0,fontSize:12}}>{user.email}</p><p style={{margin:"4px 0 0",fontSize:12}}>{user.phone || "Telefone não cadastrado"}</p></div>
            <div><small style={{color:"#7b8189"}}>IDENTIDADE</small><p style={{margin:"6px 0",fontSize:12}}>CPF {user.cpf_masked || "não cadastrado"}</p><p style={{margin:0,fontSize:11}}>{user.cpf_verified?"CPF validado":"CPF pendente"} · {user.email_verified?"e-mail validado":"e-mail pendente"} · {user.phone_verified?"telefone validado":"telefone pendente"}</p></div>
...
function founderLabel(status: UserRow["founder_status"]) {
  return ({reserved:"Reservado",active:"Ativado",expired:"Expirado",cancelled:"Cancelado",none:"Não fundador"} as const)[status];
}

function accountLabel(status: UserRow["account_status"]) {
  return ({active:"Ativo",blocked:"Bloqueado",cancelled:"Cancelado"} as const)[status];
}
